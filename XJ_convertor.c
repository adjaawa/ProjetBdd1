...
int c;
 ...
 //Il prend en paramètre un entier qui designe le nombre maximum d'emboitements
  ...
  if (!JSON_checker_done(jc)) { /* Si chaque appel de la fonction Json_checker_char a retourné 'vrai', la fonction Json_checker_done
    ...
    if (count_i == 1){
        if (strcmp(file_format, "json") == 0 || strcmp(file_format, "xml") == 0){
            if (count_h == 1 ^ count_f == 1) {
                ...
                else //Le nombre d'arguments est superieur a celui attendu
			{
